...
class _ManageCasesScreenState extends State<ManageCasesScreen> {
  ...
  /// يخزن فقط الصفوف التي تم تعديلها
  ...
  Future<void> showAddArticleDialog() async {
    ...
    try {
      final Map<String, dynamic> newArticle = {
        'article_number':
            articleNumberController.text.trim().isEmpty
                ? null
                : articleNumberController.text.trim(),
        'article_content': articleContentController.text.trim(),
        'penalty_details':
            penaltyDetailsController.text.trim().isEmpty
                ? null
                : penaltyDetailsController.text.trim(),
        'reference_url':
            referenceUrlController.text.trim().isEmpty
                ? null
                : referenceUrlController.text.trim(),
        'crime_category_id':
            crimeCategoryIdController.text.trim().isEmpty
                ? null
    ...
    return Padding(
      ...
      child: TextFormField(
        ...
        onChanged: (newValue) {
          ...
          if (column == 'crime_category_id') {
            parsedValue =
                newValue.trim().isEmpty ? null : int.tryParse(newValue);
          } else {
